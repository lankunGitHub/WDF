...
int _work(server &work, int epfd)
{
	...
	while (true)
	{
		...
		for (int k = 0; k < i; ++k)
		{
			...
			{
				char temp[7000];
				websocket.WebSocket_Recv(events[k].data.fd, outBuf, temp, 6999, 0, frame_type);
				if (frame_type == 2)
				{
					std::cout << "???" << std::endl;
					// file_store();
				...
				else if (frame_type == -1)
				{
					...
					epoll_ctl(epfd, EPOLL_CTL_DEL, events[k].data.fd, NULL);
					continue; // 继续处理下一个事件
				}
				...
				jsonmap = json.parseJOSN(boat);
				int find_type = std::stoi(jsonmap["type"]);
				std::string email, code;
				struct Info info;
				std::cout << "type:" << find_type << std::endl;
				switch (find_type)
				{
				case 0:
					std::cout << "注册" << std::endl;
					if(work.codeMap.find(json.FindJSON(jsonmap,"code")) == work.codeMap.end())
					{
						std::cout << "好家伙" << std::endl;
						jsonmap["type"] = "-1";
						code = json.ToJOSN(jsonmap);
						websocket.WebSocket_Send(events->data.fd, code.c_str(), code.length(), 0);
						continue;
					}
					code = job.SignUp(thread_connect, jsonmap, events[k].data.fd);
					change_online(work.online_clnts, code, 1);
					break;
				case 1:
					job.LogOut(jsonmap, work.online_clnts);
					break;
				case 4: 
					code = generateVerificationCode();
					email = jsonmap["email"];
					sendemail(work.ssl, code, email);
					info.email = email;
					info.time = std::chrono::system_clock::now();
					work.codeMap.insert({code, info});
				default:
			...
			if (events[k].events & (EPOLLERR | EPOLLHUP))
				{
					// 客户端连接出错或关闭
					std::cout << "Client disconnected or encountered an error" << std::endl;
					close(events[k].data.fd);
...
int server::_startServer(int &port)
{
	...
	std::shared_ptr<int[]> epfd(new int[work_num],
															[](int *p)
															{ delete[] p; });
	...
	for (int i = 0; i < work_num; ++i)
	{
		...
		std::cout << "EPFD:	" << epfd[i] << std::endl;
		this->server_pool.execute([this, epfd, i]()
															{ _work(*this, epfd[i]); });
	}
	...
	while (true)
	{
		...
		{
			clnt_sock = accept4(serv_sock, NULL, NULL, 0);
			if (clnt_sock == -1)
			{
				perror("accept4");
			}
			char recvf[1000];
			int sss = recv(clnt_sock, recvf, 999, 0);
