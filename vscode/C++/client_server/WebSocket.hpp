...
int WebSocket::getWSFrameDataHead(char msg[6], int &msgLen)
{
    if (msgLen < 2)
...
ssize_t WebSocket::WebSocket_Recv(int fd, std::vector<char> &outBuf, char *buf, int lenth, int flag, int &type)
{
    ...
    int res = len;
    char headbuf[6];
    memcpy(headbuf, buf, 6);
    getWSFrameDataHead(headbuf, res);
    len += recv(fd, buf+6, res-6, flag);
    type = getWSFrameData((char *)buf, 4095, outBuf, &res);

    //return res;
    // int len = recv(fd, buf, 4095, flag);
    // int res = len;
    // getWSFrameData(buf, len, outBuf, &res);
    // std::cout << "res:" << len << "  " << res << std::endl;
    // return res;
}
